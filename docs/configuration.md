...
| `SEARCH_RERANK_MAX_CONCURRENCY` | ⚠️ Optional | `1` | Concurrent rerank calls in flight, process-wide. Note what this does *not* do: where the reranker shares a device with the embedding model, a single in-flight rerank is already enough to slow embedding substantially, so raising or lowering this will not protect indexing throughput. What it bounds is queue depth, and therefore rerank latency under a burst. Must be `>= 1`. |
