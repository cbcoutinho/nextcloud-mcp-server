...
| `OIDC_DISCOVERY_BACKOFF_MAX` | Optional (default `15.0`) | Per-retry cap in seconds for the discovery backoff. With the defaults, worst-case startup blocks on the order of ~90s of backoff before a persistently-down IdP finally exits — size your k8s `startupProbe`/`livenessProbe` accordingly. |
